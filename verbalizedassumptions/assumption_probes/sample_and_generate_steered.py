...
def register_single_alpha_hook(model, layer_idx: int, direction: torch.Tensor):
    """
    Register ONE hook on model.model.layers[layer_idx] that subtracts alpha * direction,
    where alpha is read from a mutable dict state.
    """
    state = {"alpha": 0.0}
...
def main():
    ...
    ap.add_argument("--probe_dir", required=True, help="Directory containing validation_direction.npy (+ meta.json)")
    ...
    ap.add_argument("--batch_size", type=int, default=16, help="Batch size for generation (8B @ 200 toks: try 8-64)")
    ap.add_argument("--use_4bit", action="store_true", help="Load in 4-bit (usually slower than bf16 on big GPUs)")
    ap.add_argument("--device_map", type=str, default="auto",
                    help="e.g. auto / balanced / sequential (accelerate); use 'auto' for 70B multi-GPU")
    ap.add_argument("--trust_remote_code", action="store_true",
                    help="Set if the model requires trust_remote_code (e.g. some Qwen variants)")
    ap.add_argument("--attn_impl", default="",
                    help="Set attention implementation, e.g. 'flash_attention_2' (if supported)")
    args = ap.parse_args()
    ...
    # ----- load direction + choose layer -----
    direction_np = np.load(os.path.join(args.probe_dir, "validation_direction.npy")).astype(np.float32)
    ...
    try:
        ...
        print(f"Used layer={layer} | alphas={alphas} | batch_size={args.batch_size} | "
