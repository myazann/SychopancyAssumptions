...
def test_restrict_cells_limits_to_full_design_coordinates():
    ...
    cells = build_cells(
        personas, prompts, include_no_persona=False, restrict_cells=keep
    )
    got = {
        (
            c.persona.persona_type,
            c.persona.persona_id,
            c.prompt.prompt_type,
            c.prompt.prompt_id,
        )
        for c in cells
    }
...
def test_partial_grid_interleaves_source_order_facets_and_control():
    personas = [
        _persona("hobbies", "p1"),
        _persona("politics", "p1"),
        _persona("hobbies", "p2"),
        _persona("politics", "p2"),
    ]
...
def test_exact_design_ids_build_a_requested_union_without_resampling():
    personas = [
        _persona(facet, person)
        for person in ("p1", "p2", "p3")
        for facet in ("hobbies", "politics")
    ]
    prompts = [
    ...
    assert {cell.persona.persona_id for cell in cells} == {NO_PERSONA, "p1", "p3"}
    assert {cell.prompt.prompt_id for cell in cells} == {"q1", "q2"}
    assert len(cells) == (2 * 2 + 1) * 2 * 2
...
def test_summaries_never_pool_runs_or_control_baselines():
    ...
    model_a = labels[
        (labels.run_id == "run-model-a") & (labels.persona_type == "age")
    ].iloc[0]
    model_b = labels[
        (labels.run_id == "run-model-b") & (labels.persona_type == "age")
    ].iloc[0]
...
def test_structured_summary_uses_matching_control_and_paired_framings():
    ...
    for persona_type, persona_id, original, flipped in (
        ...
        for framing, score in (("original_post", original), ("flipped_story", flipped)):
            rows.append(
                {
    ...
    age_original = scores[
        (scores.persona_type == "age") & (scores.prompt_type == "original_post")
    ].iloc[0]
